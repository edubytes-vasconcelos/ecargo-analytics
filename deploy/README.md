...
Enquanto a permissão de SharePoint não estiver liberada, a tela de Projetos aceita upload manual de um cronograma `.xml`, `.mpp` ou `.mpt`. Arquivos do Project são convertidos automaticamente para XML com MPXJ antes da leitura das métricas. O arquivo original e o XML convertido ficam salvos no volume persistente em `DATA_DIR/project_uploads`.
