...
WORK_ITEM_IMAGE_UPLOADS = DATA_DIR / "work_item_images"
WORK_ITEM_IMAGE_UPLOADS.mkdir(parents=True, exist_ok=True)
...
WORK_ITEM_IMAGE_EXTENSIONS = {".png", ".jpg", ".jpeg", ".webp", ".gif"}
...
def _save_work_item_image(file_item) -> dict:
    original_name = Path(str(getattr(file_item, "filename", "") or "")).name
    suffix = Path(original_name).suffix.lower()
    if suffix not in WORK_ITEM_IMAGE_EXTENSIONS:
        raise RuntimeError("Formato inválido. Envie uma imagem .png, .jpg, .jpeg, .webp ou .gif.")

    image_id = uuid.uuid4().hex
    filename = f"{image_id}{suffix}"
    target = WORK_ITEM_IMAGE_UPLOADS / filename
    with target.open("wb") as fh:
        fh.write(file_item.file.read())
    return {
        "id": image_id,
        "name": original_name,
        "url": f"{BASE_PATH}/api/work-item-images/{filename}",
    }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/project-plan-image":
            ...
            self.wfile.write(body)
            return
        work_item_image_match = re.fullmatch(r"/api/work-item-images/([^/]+)", path)
        if work_item_image_match:
            image_path = (WORK_ITEM_IMAGE_UPLOADS / work_item_image_match.group(1)).resolve()
            uploads_root = WORK_ITEM_IMAGE_UPLOADS.resolve()
    ...
    def do_POST(self) -> None:
        ...
        if path == "/api/work-item-images":
            try:
                _fields, file_item = _read_form_data(self.headers, self.rfile)
                if file_item is None or not getattr(file_item, "filename", ""):
                    self._send_json({"error": "Envie uma imagem .png, .jpg, .jpeg, .webp ou .gif."}, 400)
                    return
                self._send_json(_save_work_item_image(file_item), 201)
            except Exception as exc:
                self._send_json({"error": str(exc)}, 400)
            return
